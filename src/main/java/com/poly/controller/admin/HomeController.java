...
//		}
	...
	protected void doGetHome(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		List<VideoLikedInfo> videos = statsService.findVideoLikedInfo(); //lay all video co ton tai de thong cai
		req.setAttribute("videos", videos);
		RequestDispatcher requestDispatcher = req.getRequestDispatcher("/views/admin/home.jsp");
		requestDispatcher.forward(req, resp);
	}

	// http://localhost:8080/java4asm/admin/favorites?href={videoHref}
	protected void doGetFavorites(HttpServletRequest req, HttpServletResponse resp)
		...
		if (users.isEmpty()) {
			...
			resp.setStatus(200); //200 vi co tra ve data
